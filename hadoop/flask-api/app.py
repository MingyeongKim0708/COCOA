...
def analyze_review():
    ...
    Path("/tmp/reviews").mkdir(parents=True, exist_ok=True)
    local_path = f"/tmp/reviews/review_{review_id}.txt"
    with open(local_path, "w", encoding="utf-8") as f:
        f.write(review_content)

    hdfs_path = f"/input/review_{review_id}.txt"
